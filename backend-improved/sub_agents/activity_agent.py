...
Integrates Tavily search for real-time information about activities and attractions.
"""
...
if vertex_ai_mode:
    try:
        ...
        try:
            ...
            # Fall back to absolute import
            from sub_agents import activity_agent_prompt
        ...
        if tavily_available:
            try:
                ...
                adk_tavily_tool = Tool(
                    ...
                    description="Search for real-time information about activities and attractions using Tavily search API",
                    name="tavily_search"
                )
    ...
    if tavily_available:
        try:
            # Create a simple activity search function for Direct API mode
            def search_activities_with_tavily(query, location=None):
                """
                Search for activity and attraction information using Tavily search.
                ...
                try:
                    ...
                    logger.info(f"Direct API Mode: Searching Tavily for activities: {search_query}")
                    
                    result = tavily_search(search_query)
                    
                    # Log success and result summary
                    result_len = len(str(result)) if result else 0
                    logger.info(f"Direct API Mode: Tavily activity search successful. Result length: {result_len} chars")
                    
                    return result
                except Exception as e:
                    logger.error(f"Direct API Mode: Tavily activity search error: {e}")
                    return None
                
            # Make the search function globally available
            ActivityAgent = None
            activity_tavily_search = search_activities_with_tavily
            logger.info("Direct API Mode: Tavily activity search function created and ready for use")
        except Exception as e:
            logger.error(f"Direct API Mode: Failed to create Tavily activity search function: {e}")
            ActivityAgent = None
            activity_tavily_search = None
    else:
        logger.info("Direct API Mode: Tavily search not available for activity agent")
        ActivityAgent = None
        activity_tavily_search = None

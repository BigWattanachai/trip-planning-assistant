"""
Accommodation recommendations for the Travel Agent.
Integrates Tavily search for real-time information about accommodations.
"""
...
if vertex_ai_mode:
    try:
        ...
        try:
            ...
            from . import accommodation_agent_prompt
        except (ImportError, ValueError):
            # Fall back to absolute import
            from sub_agents import accommodation_agent_prompt
        ...
        if tavily_available:
            try:
                ...
                adk_tavily_tool = Tool(
                    ...
                    description="Search for real-time information about accommodations using Tavily search API",
                    name="tavily_search"
                )
        ...
        AccommodationAgent = Agent(
            ...
            description=(
                "Recommend accommodation options based on user preferences and requirements."
            ),
            instruction=accommodation_agent_prompt.PROMPT,
            tools=tools,
    ...
    if tavily_available:
        try:
            # Create a simple accommodation search function for Direct API mode
            def search_accommodations_with_tavily(query, location=None):
                """
                Search for accommodation information using Tavily search.
                ...
                try:
                    ...
                    logger.info(f"Direct API Mode: Searching Tavily for accommodations: {search_query}")
                    
                    result = tavily_search(search_query)
                    
                    # Log success and result summary
                    result_len = len(str(result)) if result else 0
                    logger.info(f"Direct API Mode: Tavily accommodation search successful. Result length: {result_len} chars")
                    
                    return result
                except Exception as e:
                    logger.error(f"Direct API Mode: Tavily accommodation search error: {e}")
                    return None
                
            # Make the search function globally available
            AccommodationAgent = None
            accommodation_tavily_search = search_accommodations_with_tavily
            logger.info("Direct API Mode: Tavily accommodation search function created and ready for use")
        except Exception as e:
            logger.error(f"Direct API Mode: Failed to create Tavily accommodation search function: {e}")
            AccommodationAgent = None
            accommodation_tavily_search = None
    else:
        logger.info("Direct API Mode: Tavily search not available for accommodation agent")
        AccommodationAgent = None
        accommodation_tavily_search = None

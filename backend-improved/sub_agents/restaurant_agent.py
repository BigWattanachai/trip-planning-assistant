...
Integrates Tavily search for real-time information about restaurants and dining options.
"""
...
if vertex_ai_mode:
    try:
        ...
        try:
            ...
            # Fall back to absolute import
            from sub_agents import restaurant_agent_prompt
        ...
        if tavily_available:
            try:
                ...
                adk_tavily_tool = Tool(
                    ...
                    description="Search for real-time information about restaurants and dining options using Tavily search API",
                    name="tavily_search"
                )
    ...
    if tavily_available:
        try:
            # Create a simple restaurant search function for Direct API mode
            def search_restaurants_with_tavily(query, location=None):
                """
                Search for restaurant and dining information using Tavily search.
                ...
                try:
                    ...
                    logger.info(f"Direct API Mode: Searching Tavily for restaurants: {search_query}")
                    
                    result = tavily_search(search_query)
                    
                    # Log success and result summary
                    result_len = len(str(result)) if result else 0
                    logger.info(f"Direct API Mode: Tavily restaurant search successful. Result length: {result_len} chars")
                    
                    return result
                except Exception as e:
                    logger.error(f"Direct API Mode: Tavily restaurant search error: {e}")
                    return None
                
            # Make the search function globally available
            RestaurantAgent = None
            restaurant_tavily_search = search_restaurants_with_tavily
            logger.info("Direct API Mode: Tavily restaurant search function created and ready for use")
        except Exception as e:
            logger.error(f"Direct API Mode: Failed to create Tavily restaurant search function: {e}")
            RestaurantAgent = None
            restaurant_tavily_search = None
    else:
        logger.info("Direct API Mode: Tavily search not available for restaurant agent")
        RestaurantAgent = None
        restaurant_tavily_search = None

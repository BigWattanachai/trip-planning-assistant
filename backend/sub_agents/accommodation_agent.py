...
if os.getenv("GOOGLE_GENAI_USE_VERTEXAI", "0").lower() in ("1", "true", "yes"):
    try:
        ...
        try:
            ...
            # Fall back to absolute import
            from backend.sub_agents import accommodation_agent_prompt
        ...
        logger.info("Accommodation agent created successfully")
        
    except ImportError as e:
        logger.error(f"Failed to import ADK components for accommodation agent: {e}")
        AccommodationAgent = None
else:
    logger.info("Direct API Mode: Accommodation agent not loaded")
    AccommodationAgent = None
